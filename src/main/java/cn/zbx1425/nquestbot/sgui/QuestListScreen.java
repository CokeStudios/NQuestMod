...
import cn.zbx1425.nquestbot.data.quest.QuestTier;
...
public class QuestListScreen extends TabbedItemListGui<Quest, Pair<String, QuestCategory>, Void> {
    ...
    public QuestListScreen(ServerPlayer player, BaseSlotGui parent, Consumer<Quest> callback) {
        super(MenuType.GENERIC_9x4, player, parent,
                NQuestBot.INSTANCE.questCategories.entrySet().stream()
    ...
    protected CompletableFuture<Pair<List<Quest>, Integer>> supplyItems(int offset, int limit) {
        ...
        List<Quest> filteredQuests = NQuestBot.INSTANCE.questDispatcher.quests.values().stream()
                .filter(q -> selectedPrimaryTab.getKey().equals(q.category))
                .sorted(Comparator.comparingInt(q -> {
                    QuestCategory cat = NQuestBot.INSTANCE.questCategories.get(q.category);
    ...
    protected GuiElementBuilder createElementForItem(Quest item, int index) {
        QuestCategory cat = NQuestBot.INSTANCE.questCategories.get(item.category);
        QuestTier tier = null;
